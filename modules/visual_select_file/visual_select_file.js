...
(function($) {
  ...
  Drupal.visualSelectFile.ckeditorEmbeddable = function(file, format, editor, owner) {
    ...
    var
      args = format.split(':'),
      format = Drupal.visualSelectFile.overrideSelectedFormat(args.shift(), file);
  ...
  Drupal.visualSelectFile.overrideSelectedFormat = function(format, file) {
    if (!file[3].match(/\.(jpe?g|gif|png|svg)$/i)) {
      return 'link';
    }
